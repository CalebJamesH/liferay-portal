...
const buildYearFilter = (searchBuilder: SearchBuilder, year: number) => {
...
const buildCurrentAndLastYearFilters = (searchBuilder: SearchBuilder) => {
	const currentYear = new Date().getFullYear();

	return {
		currentYear: buildYearFilter(searchBuilder, currentYear),
		lastYear: buildYearFilter(searchBuilder, currentYear - 1),
	};
};
